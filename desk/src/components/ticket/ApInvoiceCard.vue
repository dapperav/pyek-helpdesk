<template>
  <!-- AP invoice working card (mobile + desktop ticket detail). Renders only for
       ...
       This is the ONLY editable surface for the AP fields — the "Ticket Info" panel
       that used to duplicate them is hidden on AP. Every value here writes straight
       to the ticket, and the Intacct filename is derived from those live fields, so a
       correction renames the download immediately. Also hosts the whole action stack:
       verify, assign, download, priority, and a quick note. -->
  <div v-if="isAP" class="px-5 pt-4">
    <div class="rounded-xl border border-outline-gray-2 bg-surface-white p-3.5">
      ...
      <!-- Fields — every row click-to-edit, and now the ONLY place these live. The
           "Ticket Info" panel used to show the same nine values as editable form
           controls, so each appeared twice (Terms and the invoice number three times);
           that panel is hidden on AP now. Rows always render even when empty, because
           a blank "Set" is how you discover you still owe a value — the old card hid
           empty rows and the verify gate then failed for no visible reason. Set in
           text-lg rather than the 14px it used: these are what Nedra reads off against
           the PDF all day, so they're the one place in the sidebar worth the size. -->
      ...
      <!-- Actions, flat: Verify, Assign, Download, Priority — Mark's order. The
           "Invoice files"/"Workflow" headings are gone with it; they existed to say
           where the working surface ended, and putting the two actions Nedra takes on
           every ticket at the top says it better than a label did. -->
