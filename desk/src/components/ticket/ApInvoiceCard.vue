<template>
  <!-- AP invoice working card (mobile ticket detail). Renders only for AP
       tickets (the ap_vendor field is present); on IT/HR it renders nothing,
       so it's safe on the shared component. At-a-glance invoice summary +
       the copyable Intacct filename + a one-tap jump to the PDF. -->
  <div v-if="isAP" class="px-5 pt-4">
    <div class="rounded-xl border border-outline-gray-2 bg-surface-white p-3.5">
      ...
      <!-- View the invoice PDF (jumps to the Emails tab where attachments live) -->
      <button
...
// ap_proposed_filename + ap_duplicate are enricher-set fields the detail
// payload may not include, so self-fetch them once (keyed on the ticket) only
// when the filename isn't already on the doc.
